...
const CARDS_CONFIG = {
  ...
  contatos: {
    id: 'contatos',
    title: "Contatos",
    link: "/plataforma/contatos",
    img_src: "/contatos.svg",
    tipo_perfil: [1, 2]
  },
  projetos:{
    id: 'projetos',
    title: "Projetos",
    link: "/plataforma/projetos",
    img_src: "/projetos.svg",
    tipo_perfil: [1, 2]
  },
  cursos: {
    id: 'cursos',
...
export async function registerUser(formData) {
    ...
    const {data, error} = await supabase.auth.signUp({
        ...
        options: {
            ...
            emailRedirectTo: `${process.env.NEXT_PUBLIC_BASE_URL}/auth/callback?next=/plataforma`
        }
    });
    return {data, error};
}
...
export async function contato(formData) {
    ...
    const { data, error } = await supabase
        ...
        .insert({
            ...
            is_comercial: formData.is_comercial
        });
    return { data, error };
}
...
export async function canChangeProfileImageForUser() {
  ...
  return tipo_usuario === 1 || tipo_usuario === 2 || tipo_usuario === 3;
}
...
export async function lerdadoshistoricos(){
    const supabase = await createSupabaseServer();
    const {data,error} = await supabase
        .from('equipe_historica')
        .select('*');
        if (error) {
            console.error("Erro ao buscar dados da equipe histórica:", error);
        }
    return {data, error};
}
...
export async function redefinirSenha(email) {
    ...
    const { data, error } = await supabase.auth.resetPasswordForEmail(email, {
        // O redirectTo DEVE estar dentro de options
            redirectTo: `${process.env.NEXT_PUBLIC_BASE_URL}/auth/callback?next=/nova-senha`
    });
    return { data, error };
}

export async function trocarSenha(password) {
    const supabase = await createSupabaseServer();
...
export async function lerctt(){
    const supabase = await createSupabaseServer();
    const {data,error} = await supabase
        .from('Contato')
        .select('id, nome, email, areasDeInteresse, telefone, mensagem, tipoDeProjeto, ano_entrada, curso, is_comercial');
    if (error) {
...
export async function lerprojetos(){
    const supabase = await createSupabaseServer();
    const {data,error} = await supabase
        .from('Projeto')
        .select('id,slug, area, titulo, resumo, parceiros, imagem, cat, problema, confeccao,resultados,anoinicio,anofim,links');
    if (error) {
...
export async function insertprojetos(titulo, slug, resumo, problema, confeccao, resultado, areas, categorias, links, ano_inicio, ano_termino, foto_principal, fotos_parceiros) {
    const supabase = await createSupabaseServer();
    let Principal = `${slug}/principal.jpeg`;
    let parceirosPath = null; 

    const buffer = await foto_principal.arrayBuffer();
    const { error: uploadError } = await supabase
        .storage
        .from('imagens-projetos')
        .upload(Principal, Buffer.from(buffer), { 
            upsert: true,
            contentType: foto_principal.type
        });
    ...
    if (uploadError) { 
      ...
      // Vamos pegar a URL pública da imagem principal para armazenar no banco
      const { data: { publicUrl } } = supabase
        .storage
        .from('imagens-projetos')
        .getPublicUrl(Principal);
        Principal = publicUrl; // Armazenar a URL pública no banco
    }

    if (fotos_parceiros && fotos_parceiros.size > 0) {
        parceirosPath = `${slug}/parceiros.png`;
        const buffer2 = await fotos_parceiros.arrayBuffer();
        
        const { error: uploadError2 } = await supabase
            .storage
            .from('imagens-projetos')
            .upload(parceirosPath, Buffer.from(buffer2), { 
                upsert: true,
                contentType: fotos_parceiros.type
            });

        if (uploadError2) {
            return { data: null, error: uploadError2.message };
        }
        else {
            // Gerar URL pública para a imagem dos parceiros
            const { data: { publicUrl: parceirosPublicUrl } } = supabase
                .storage
                .from('imagens-projetos')
                .getPublicUrl(parceirosPath);
            parceirosPath = parceirosPublicUrl; // Armazenar a URL pública no banco
        }
    }
    ...
    const { data, error: dbError } = await supabase
        ...
        .insert({
            ...
            anofim: ano_termino,
            imagem: Principal,
            parceiros: parceirosPath 
        })
        .select(); 

    if (dbError) {
        console.error("Erro ao inserir projeto:", dbError);
        return { data: null, error: dbError.message };
    }
    return { data, error: null };
}

export async function removerprojeto(slug) {

    const supabase = await createSupabaseServer();
    const caminhosParaDeletar = [
        `${slug}/principal.jpeg`,
        `${slug}/parceiros.png`
    ];
    ...
    if (removeError) {
        console.error("Erro ao remover imagens:", removeError);
    } else {
        console.log("Imagens removidas:", data);
    }
...
export async function editarprojeto(titulo, slug, resumo, problema, confeccao, resultado, areas, categorias, links, ano_inicio, ano_termino, foto_principal, fotos_parceiros) {
    const supabase = await createSupabaseServer();
    
    // 1. Caminhos fixos
    const pathPrincipal = `${slug}/principal.jpeg`;
    const pathParceiros = `${slug}/parceiros.png`;

    // 2. Inicializar variáveis de URL
    let urlFinalPrincipal = "";
    let urlFinalParceiros = null;

      // --- Lógica Principal ---
    if (foto_principal && foto_principal.size > 0) {
        const buffer = await foto_principal.arrayBuffer();
        await supabase.storage
            .from('imagens-projetos')
            .upload(pathPrincipal, Buffer.from(buffer), { 
                upsert: true,
                contentType: 'image/jpeg' 
            });
    }

    // BUSCA A URL E ADICIONA UM TIMESTAMP
    const { data: pData } = supabase.storage.from('imagens-projetos').getPublicUrl(pathPrincipal);
    // Adicionamos ?t= + milissegundos atuais para forçar o refresh
    urlFinalPrincipal = `${pData.publicUrl}?t=${new Date().getTime()}`;

    // --- Lógica Parceiros ---
    if (fotos_parceiros && fotos_parceiros.size > 0) {
        const buffer = await fotos_parceiros.arrayBuffer();
        await supabase.storage
            .from('imagens-projetos')
            .upload(pathParceiros, Buffer.from(buffer), { 
                upsert: true,
                contentType: 'image/png'
            });
        
        const { data: prData } = supabase.storage.from('imagens-projetos').getPublicUrl(pathParceiros);
        urlFinalParceiros = `${prData.publicUrl}?t=${new Date().getTime()}`;
    }

    // 3. Agora sim, faz o UPDATE no banco com as URLs corretas
    const { data, error } = await supabase
        .from('Projeto')
        .update({
            titulo,
            ...
            anoinicio: ano_inicio,
            anofim: ano_termino,
            imagem: urlFinalPrincipal,
            parceiros: urlFinalParceiros
        })
        .eq('slug', slug); // Usando slug como chave
